...
const Checkout = () => {
  ...
  useEffect(() => {
    const checkStripe = () => {
      if (!stripe && !elements) {
        ...
        console.warn('🚫 Stripe appears to be blocked by ad blocker');
      } else if (stripe && elements) {
        setStripeBlocked(false);
        console.log('✅ Stripe is working properly');
      }
    };
    
    // Check after a short delay to allow Stripe to load
    const timer = setTimeout(checkStripe, 2000);
  ...
  const handleRealPayment = async () => {
    if (!stripe || !elements) {
      console.warn('⚠️ Stripe not available - likely blocked by ad blocker');
      setPaymentError('Payment system blocked by ad blocker. Please disable ad blocker or use Demo Payment.');
      return;
    }
            ...
            {stripeBlocked && (
              <div className="bg-yellow-900 border border-yellow-600 p-4 rounded-lg mb-4">
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="text-yellow-300 font-semibold">Ad Blocker Detected</p>
                    <p className="text-yellow-200 text-sm">Payment system is blocked. Please disable ad blocker or use Demo Payment below.</p>
                  </div>
                </div>
              </div>
            )}
